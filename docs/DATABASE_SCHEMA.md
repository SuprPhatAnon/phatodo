...
Completion-related fields are intentionally explicit so the server can enforce accountability: tasks and epics can record who owns work, who completed it, what evidence was used, and which summary or checkpoint comment was left behind. Plain narrative fields like `description` and `completion_summary` stay as text, while `acceptance_criteria` and `completion_evidence` stay as ordered JSON lists because they can hold multiple items and the order matters.
